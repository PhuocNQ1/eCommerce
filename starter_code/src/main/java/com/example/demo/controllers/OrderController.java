...
public class OrderController {
    ...
    public ResponseEntity<UserOrder> submit(@PathVariable String username) {
        ...
        if (user == null) {
            log.warn("User with username '{}' not found. Order submission failed.", username);
        ...
        log.info("Order submitted successfully for user: {}, Order ID: {}", username, order.getId());
    ...
    public ResponseEntity<List<UserOrder>> getOrdersForUser(@PathVariable String username) {
        ...
        User user = userRepository.findByUsername(username);
        if (user == null) {
            log.warn("User with username '{}' not found. Unable to retrieve order history.", username);
            return ResponseEntity.notFound().build();
        }
